...
export interface DebtPayment {
   ...
   *  history. */
  paid: boolean;
  /** True when the line was generated by the loan's repayment plan. Changing
   *  the plan rebuilds these and leaves hand-added payments alone. */
  fromPlan?: boolean;
}

export type PlanEvery =
  | 'once'
  | 'daily'
  | 'weekly'
  | 'biweekly'
  ...
  | 'quarterly'
  | 'custom';
export type PlanUnit = 'days' | 'weeks' | 'months';

/** How a loan is being paid back: the recipe the dated payment lines are built
 *  from. Stored on the loan so re-opening Edit shows what was chosen instead of
 *  resetting to "every month". */
...
export interface Debt {
  ...
  /** Set when the payment lines came from a repayment plan rather than being
   *  added one at a time. */
  plan?: RepaymentPlan;
  verified: boolean;
  note?: string;
