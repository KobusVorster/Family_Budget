...
The simulator is located at `.claude/vc2-essentials/tools/safer.py`. It produces detailed HTML reports with quarterly cash flow projections, Safer Amount evolution, IRR calculations, and return waterfall analysis with embedded charts.
...
```bash
python .claude/vc2-essentials/tools/safer.py --scenario <scenario-name> --output <output-path>
```
...
```bash
python .claude/vc2-essentials/tools/safer.py \
    --investment <amount> \
    --valuation-cap <cap> \
